...
> UsageCount CountUsageV1UsageCountGet(ctx).StartDate(startDate).EndDate(endDate).UserId(userId).Status(status).StatusCode(statusCode).Model(model).Endpoint(endpoint).Provider(provider).Source(source).SourceLabel(sourceLabel).ApiKeyId(apiKeyId).Priced(priced).Tool(tool).CountsTowardBudget(countsTowardBudget).RequestGroupId(requestGroupId).Execute()
...
> []UsageEntry ListUsageV1UsageGet(ctx).StartDate(startDate).EndDate(endDate).UserId(userId).Status(status).StatusCode(statusCode).Model(model).Endpoint(endpoint).Provider(provider).Source(source).SourceLabel(sourceLabel).ApiKeyId(apiKeyId).Priced(priced).Tool(tool).CountsTowardBudget(countsTowardBudget).RequestGroupId(requestGroupId).Skip(skip).Limit(limit).Execute()
...
> interface{} UsageSummaryCsvV1UsageSummaryCsvGet(ctx).StartDate(startDate).EndDate(endDate).UserId(userId).Status(status).StatusCode(statusCode).Model(model).Endpoint(endpoint).Provider(provider).Source(source).SourceLabel(sourceLabel).ApiKeyId(apiKeyId).Priced(priced).Tool(tool).CountsTowardBudget(countsTowardBudget).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.UsageAPI.UsageSummaryCsvV1UsageSummaryCsvGet(context.Background()).StartDate(startDate).EndDate(endDate).UserId(userId).Status(status).StatusCode(statusCode).Model(model).Endpoint(endpoint).Provider(provider).Source(source).SourceLabel(sourceLabel).ApiKeyId(apiKeyId).Priced(priced).Tool(tool).CountsTowardBudget(countsTowardBudget).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `UsageAPI.UsageSummaryCsvV1UsageSummaryCsvGet``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `UsageSummaryCsvV1UsageSummaryCsvGet`: interface{}
	fmt.Fprintf(os.Stdout, "Response from `UsageAPI.UsageSummaryCsvV1UsageSummaryCsvGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiUsageSummaryCsvV1UsageSummaryCsvGetRequest struct via the builder pattern
...
> UsageSummary UsageSummaryV1UsageSummaryGet(ctx).StartDate(startDate).EndDate(endDate).UserId(userId).Status(status).StatusCode(statusCode).Model(model).Endpoint(endpoint).Provider(provider).Source(source).SourceLabel(sourceLabel).ApiKeyId(apiKeyId).Priced(priced).Tool(tool).CountsTowardBudget(countsTowardBudget).Bucket(bucket).Dimensions(dimensions).Execute()
