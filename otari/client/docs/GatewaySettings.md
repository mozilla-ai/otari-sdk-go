...
**ModelDiscovery** | **bool** |  | 
**RequirePricing** | **bool** |  | 
**SecretKeyConfigured** | **bool** | Whether OTARI_SECRET_KEY is set on the server. Provider credentials are encrypted at rest with it, so the dashboard disables adding stored providers when it is unset. | 
**Version** | **string** |  | 
