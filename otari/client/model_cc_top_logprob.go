/*
otari

Otari, an OpenAI-compatible LLM gateway with API key management

API version: 0.0.0-dev
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
	"fmt"
)

// checks if the CCTopLogprob type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CCTopLogprob{}

// CCTopLogprob struct for CCTopLogprob
type CCTopLogprob struct {
	Token                string  `json:"token"`
	Bytes                []int32 `json:"bytes,omitempty"`
	Logprob              float32 `json:"logprob"`
	AdditionalProperties map[string]interface{}
}

type _CCTopLogprob CCTopLogprob

// NewCCTopLogprob instantiates a new CCTopLogprob object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCCTopLogprob(token string, logprob float32) *CCTopLogprob {
	this := CCTopLogprob{}
	this.Token = token
	this.Logprob = logprob
	return &this
}

// NewCCTopLogprobWithDefaults instantiates a new CCTopLogprob object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCCTopLogprobWithDefaults() *CCTopLogprob {
	this := CCTopLogprob{}
	return &this
}

// GetToken returns the Token field value
func (o *CCTopLogprob) GetToken() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Token
}

// GetTokenOk returns a tuple with the Token field value
// and a boolean to check if the value has been set.
func (o *CCTopLogprob) GetTokenOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Token, true
}

// SetToken sets field value
func (o *CCTopLogprob) SetToken(v string) {
	o.Token = v
}

// GetBytes returns the Bytes field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CCTopLogprob) GetBytes() []int32 {
	if o == nil {
		var ret []int32
		return ret
	}
	return o.Bytes
}

// GetBytesOk returns a tuple with the Bytes field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CCTopLogprob) GetBytesOk() ([]int32, bool) {
	if o == nil || IsNil(o.Bytes) {
		return nil, false
	}
	return o.Bytes, true
}

// HasBytes returns a boolean if a field has been set.
func (o *CCTopLogprob) HasBytes() bool {
	if o != nil && !IsNil(o.Bytes) {
		return true
	}

	return false
}

// SetBytes gets a reference to the given []int32 and assigns it to the Bytes field.
func (o *CCTopLogprob) SetBytes(v []int32) {
	o.Bytes = v
}

// GetLogprob returns the Logprob field value
func (o *CCTopLogprob) GetLogprob() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.Logprob
}

// GetLogprobOk returns a tuple with the Logprob field value
// and a boolean to check if the value has been set.
func (o *CCTopLogprob) GetLogprobOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Logprob, true
}

// SetLogprob sets field value
func (o *CCTopLogprob) SetLogprob(v float32) {
	o.Logprob = v
}

func (o CCTopLogprob) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CCTopLogprob) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["token"] = o.Token
	if o.Bytes != nil {
		toSerialize["bytes"] = o.Bytes
	}
	toSerialize["logprob"] = o.Logprob

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *CCTopLogprob) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"token",
		"logprob",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varCCTopLogprob := _CCTopLogprob{}

	err = json.Unmarshal(data, &varCCTopLogprob)

	if err != nil {
		return err
	}

	*o = CCTopLogprob(varCCTopLogprob)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "token")
		delete(additionalProperties, "bytes")
		delete(additionalProperties, "logprob")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableCCTopLogprob struct {
	value *CCTopLogprob
	isSet bool
}

func (v NullableCCTopLogprob) Get() *CCTopLogprob {
	return v.value
}

func (v *NullableCCTopLogprob) Set(val *CCTopLogprob) {
	v.value = val
	v.isSet = true
}

func (v NullableCCTopLogprob) IsSet() bool {
	return v.isSet
}

func (v *NullableCCTopLogprob) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCCTopLogprob(val *CCTopLogprob) *NullableCCTopLogprob {
	return &NullableCCTopLogprob{value: val, isSet: true}
}

func (v NullableCCTopLogprob) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCCTopLogprob) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
