...
type Content2 struct {
	ArrayOfContent2AnyOfInner *[]Content2AnyOfInner
	String                    *string
}
...
func (dst *Content2) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into ArrayOfContent2AnyOfInner
	err = json.Unmarshal(data, &dst.ArrayOfContent2AnyOfInner)
	if err == nil {
		jsonArrayOfContent2AnyOfInner, _ := json.Marshal(dst.ArrayOfContent2AnyOfInner)
		if string(jsonArrayOfContent2AnyOfInner) == "{}" { // empty struct
			dst.ArrayOfContent2AnyOfInner = nil
		} else {
			return nil // data stored in dst.ArrayOfContent2AnyOfInner, return on the first match
		}
	} else {
		dst.ArrayOfContent2AnyOfInner = nil
	}
...
func (src Content2) MarshalJSON() ([]byte, error) {
	if src.ArrayOfContent2AnyOfInner != nil {
		return json.Marshal(&src.ArrayOfContent2AnyOfInner)
	}
